...
#img1 <- readPNG("IMG_1553.png", info = TRUE)
#grid.raster(img1, hjust = 0.47 , height = 1.02)
```

```{r picture1, echo=FALSE, out.width="100%"}
knitr::include_graphics("/img/Spurnebau.jpg", error = FALSE)
```
*Above:* A photo I took from the North Stand at the **Tottenham** **Hotspur** **Stadium** in 2019 after a 1-0 win against Brighton, in which **Christian** **Eriksen** scored the only goal.
...
```{r, lifeExp_one_country, echo=FALSE}
...
```{r, lifeExp_one_country_with_label, echo=FALSE}
plot1<- plot1 +
...
```{r lifeExp_one_continent, echo=FALSE}
...
```{r lifeExp_facet_by_continent, echo=FALSE}
...
```{r brexit_histogram, warning=FALSE, message=FALSE, echo=FALSE}
...
```{r brexit_immigration_correlation, echo=FALSE}
brexit_results %>% 
...
```{r load_animal_rescue_data, warning=FALSE, message=FALSE, echo=FALSE}
...
```{r, instances_by_calendar_year, echo=FALSE}
...
```{r, animal_group_percentages, echo=FALSE}
animal_rescue %>% 
...
```{r, parse_incident_cost,message=FALSE, warning=FALSE, echo=FALSE}
...
```{r, stats_on_incident_cost,message=FALSE, warning=FALSE, echo=FALSE}
...
```{r, plots_on_incident_cost_by_animal_group,message=FALSE, warning=FALSE, echo=FALSE}
